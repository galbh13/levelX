...
// ── Coach chat (per-player DIRECT chat with the admin) ───────────────────────

// A private 1-on-1 chat between ONE player and the coach (admin) — distinct from
// the per-group community chat above. The conversation is keyed to the player
// (`player_id` = the channel); `sender_id` is who wrote each line. A message is
// "from the coach" iff sender_id !== player_id. Same 7-day ephemeral policy as the
// group chat (CHAT_RETENTION_DAYS). See migration 20260722_coach_chat.sql.

// One player's coach-chat, oldest→newest, capped at MESSAGE_LIMIT (newest kept).
export async function fetchCoachMessages(playerId) {
  if (!playerId) return [];
  const { data } = await supabase
    .from('coach_messages')
...
// The newest message in a player's coach-chat (or null) — drives the preview on
// the Community tab's COACH card.
export async function latestCoachMessage(playerId) {
...
// Post a message into a player's channel. `senderId` is the author — the player
// themselves (player screen) or an admin (coach reply). RLS enforces that a
// player may only write their own channel as themselves; the admin-all policy
// covers coach replies.
export async function sendCoachMessage(playerId, senderId, body) {
  const text = (body ?? '').trim();
  if (!playerId || !senderId || !text) return null;
  const { data, error } = await supabase
    .from('coach_messages')
    .insert({ player_id: playerId, sender_id: senderId, body: text.slice(0, 1000) })
    .select('id, player_id, sender_id, body, created_at')
    .single();
  if (error) throw error;
  return data;
...
// Delete a message. A player may unsend their OWN; an admin may delete any.
export async function deleteCoachMessage(messageId) {
  const { error } = await supabase.from('coach_messages').delete().eq('id', messageId);
  if (error) throw error;
}

// Sweep a player's coach-chat messages older than the retention window (client-
// side, on chat load — same philosophy as the group-chat purge). Best-effort.
export async function purgeExpiredCoachMessages(playerId) {
  if (!playerId) return;
  const cutoff = new Date(Date.now() - CHAT_RETENTION_DAYS * 24 * 60 * 60 * 1000).toISOString();
  try {
    await supabase
    ...
    console.error('[community] purgeExpiredCoachMessages:', e);
  }
}

// ── internal ─────────────────────────────────────────────────────────────────

// Fold flat member/challenge rows into per-group counts on each group object.
function annotate(groups, memberRows, challengeRows) {
  const memberCount = tally(memberRows);
  const challengeCount = tally(challengeRows);
  return groups.map(g => ({
    ...g,
    memberCount: memberCount[g.id] ?? 0,
    challengeCount: challengeCount[g.id] ?? 0,
  }));
}

function tally(rows) {
  const out = {};
  rows.forEach(r => { out[r.group_id] = (out[r.group_id] ?? 0) + 1; });
  return out;
}
