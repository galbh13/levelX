-- =============================================================================
-- Migration: Class I side quest — archer_pull_up (Tier 2)
-- =============================================================================
--
-- Data-only. Adds a new Tier 2 side chain in Class I. Tier is expressed
-- structurally (same pattern as 20260522_class1_side_quests.sql): order_index 0
-- is a cross-tier convergence node whose prerequisites are L = leaves of every
-- branch of every Tier 1 side chain (frog, headstand).
--
...
WHERE class_id = (SELECT id FROM classes WHERE name = 'Class I')
  AND quest_type = 'side'
  AND chain = 'archer_pull_up';

-- ─── 1. Insert archer_pull_up (single linear branch, 5 nodes) ─────────────────

INSERT INTO class_quests
  (id, class_id, quest_type, chain, branch, order_index, name, lvl_reward, is_convergence, prerequisites)
SELECT
  gen_random_uuid(), cls.id, 'side', 'archer_pull_up',
  v.branch::text, v.ord::int, v.name::text, v.reward::int, v.conv::boolean, '{}'::uuid[]
FROM (SELECT id FROM classes WHERE name = 'Class I') cls,
(VALUES
  ('main', 0, '12 Pull-ups',                 0, true),
  ('main', 1, '8 Wide Pull-ups',             0, false),
  ('main', 2, '1 Archer Pull-up (each arm)', 0, false),
  ('main', 3, '2 Archer Pull-ups',           0, false),
  ('main', 4, '6 Archer Pull-ups',           0, false)
...
WHERE cq.class_id      = cls.id
  ...
  AND cq.chain         = 'archer_pull_up'
  AND prev.chain       = 'archer_pull_up'
...
-- ─── Verification ─────────────────────────────────────────────────────────────
-- a) archer_pull_up/main has 5 rows.
-- b) order_index 0 ('12 Pull-ups') is_convergence=true, prereq length = 3
