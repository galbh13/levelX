...
| `source_template_id` | uuid | **Where this copy came from.** FK → `gallery_example_workouts(id)` `ON DELETE SET NULL`; `NULL` = authored directly for the player (nothing to revert to). Stamped by `importGalleryWorkout()`. Deleting the library program leaves the player's copy intact, it just loses the ability to revert. Added in `migrations/20260830_workout_template_link.sql` (which also backfills it by unambiguous title match). |
| `customized_at` | timestamptz | **Last time a coach edited this copy away from its template.** `NULL` = still matches the library version. Stamped by `WorkoutEditScreen`'s save; cleared by the revert. `source_template_id` + `customized_at` both set = My Workouts shows **RETURN TO NORMAL**. Same migration. |
| `created_at` | timestamptz | Auto |
